...
from train_helpers import set_up_hyperparams, load_vaes, load_opt, save_model, update_ema
...
def train_loop(H, data_train, data_valid, preprocess_fn, vae, ema_vae, logprint):
    ...
    #train_sampler = DistributedSampler(data_train, num_replicas=None, rank=None)
...
def evaluate(H, ema_vae, data_valid, preprocess_fn):
    stats_valid = []
    #valid_sampler = DistributedSampler(data_valid, num_replicas=None, rank=None)
...
def write_images(H, ema_vae, viz_batch_original, viz_batch_processed, fname, logprint):
    ...
    with torch.no_grad():
        ims = ((ema_vae.forward_uncond_samples(64))/255.).transpose(0,3,1,2)
    save_image(torch.from_numpy(ims), fname)
